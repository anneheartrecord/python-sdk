...
class Result(MCPModel):
    ...
    2026-07-28 emission; handlers normally never set it.
...
class ElicitRequestURLParams(RequestParams):
    ...
    New in protocol 2025-11-25: earlier schemas define no URL mode, but
    serialize_for still emits the plain dump on earlier sessions — sending URL
    mode only to peers that understand it is the session layer's capability
    and version gating, not serialization's.
    """
