...
One of the two wire-shape surfaces behind ``mcp.types.wire``: the boundary
validates every 2026-07-28 emission through these models as a check — the
emitted bytes are always the monolith dump, never a re-dump of a model here —
and a monolith type with no counterpart in this package has no 2026-07-28
wire form at all.

Initially generated from schema/draft/schema.json @ 6d441518de8a9d5adbab0b10a76a667a63f90665 by
...
rationale. The classes kept open are commented in place. Because unknown keys
are ignored, a caller-set field this schema does not define can never fail
the boundary's validation check: requiredness and value shapes are what these
models enforce.
...
their values are caller data, never reshaped by the boundary.
...
# schema.json shape verbatim; the parity test pins this alias separately.
