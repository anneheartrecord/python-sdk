...
One of the two wire-shape surfaces behind ``mcp.types.wire``, and the one
that serves every protocol version through 2025-11-25: the schemas from
2024-11-05 to 2025-11-25 evolve strictly additively (each defines a subset of
the types and fields here), so the newest of them describes them all.
``tests/types/test_version_model_parity.py`` proves that coverage against the
generated oracle of each served schema. The wire boundary never validates an
emission through these models — emission for the served versions is the plain
monolith dump — so this package's job is to be the reviewable record of the
2025-11-25 wire shapes and the anchor of that coverage proof.

Initially generated from schema/2025-11-25/schema.json @ 6d441518de8a9d5adbab0b10a76a667a63f90665 by
...
class ElicitResult(WireModel):
    ...
    # oracle keeps the rendering verbatim and the parity test pins this
...
class NumberSchema(WireModel):
    ...
    # oracle keeps the rendering verbatim and the parity test pins these
