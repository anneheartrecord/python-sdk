...
against the method sets derived from the generated spec oracles — all five
versions — and from the committed surface packages, each at its own version
(equality, not subset — a silently omitted method must fail).
"""
...
# The generated spec oracle module for every known version.
...
# The two committed surface packages, each compared at its own version (the
# 2025-11-25 surface serves the older versions too, but its unions are the
# 2025-11-25 method sets; the per-version sets live in the tables and the
# oracles).
_SURFACE_PACKAGES: dict[str, str] = {
    "2025-11-25": "v2025_11_25",
    "2026-07-28": "v2026_07_28",
}
...
def test_every_registry_version_is_served_by_a_surface() -> None:
    """Two surfaces cover the whole registry: every version through
    2025-11-25 is served by the 2025-11-25 surface package (those schemas
    evolve strictly additively — proven in test_version_model_parity), and
    2026-07-28 by its own."""
    assert wire._SURFACE_MODULES == {
    ...
    assert set(wire._SURFACE_MODULES) == set(wire.KNOWN_PROTOCOL_VERSIONS)
...
@pytest.mark.parametrize("version", sorted(_SURFACE_PACKAGES))
@pytest.mark.parametrize("table_name", sorted(_TABLES))
def test_table_equals_surface_package_union_methods(table_name: str, version: str) -> None:
    """Each surface's table entry equals the method set derived from that
    committed surface package's direction union, minus the task request
    methods."""
    package = importlib.import_module(f"mcp.types.{_SURFACE_PACKAGES[version]}")
...
def test_table_equals_spec_oracle_union_methods(table_name: str, version: str) -> None:
    ...
    oracle = importlib.import_module(f"tests.spec_oracles.{_ORACLES[version]}")
